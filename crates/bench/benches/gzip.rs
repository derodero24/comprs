...
fn bench_gzip(c: &mut Criterion) {
    ...
    c.bench_function("gzip compress patterned 10KB", |b| {
        b.iter(|| compress(&p10k, 6))
    });
    c.bench_function("gzip compress patterned 1MB", |b| {
        b.iter(|| compress(&p1m, 6))
    });
    c.bench_function("gzip compress random 10KB", |b| {
        b.iter(|| compress(&r10k, 6))
    });
    c.bench_function("gzip compress random 1MB", |b| b.iter(|| compress(&r1m, 6)));

    c.bench_function("gzip decompress patterned 10KB", |b| {
        b.iter(|| decompress(&p10k_c))
    });
    c.bench_function("gzip decompress patterned 1MB", |b| {
        b.iter(|| decompress(&p1m_c))
    });
    c.bench_function("gzip decompress random 10KB", |b| {
        b.iter(|| decompress(&r10k_c))
    });
    c.bench_function("gzip decompress random 1MB", |b| {
